...
for i in idxpdfs:
  ...
  for j in range(0,maxfile):
    ...
    for event in mychain:
      ...
      #get this variable
      lhewts = event.LHE_weights_scale_wgt
      if i == 5:
          lhewtcentral = float(lhewts[2])
      elif i == 6:
          lhewtcentral = float(lhewts[3])
...
#      lhewt = lhewts[i-1]
      if i == 5:
        evtwtpdfUp += float(lhewts[4]) 
      elif i== 6:
        evtwtpdfLo += float(lhewts[5])
...
#        if (cat=="LL" and passLL and not passTT) or (cat=="TT" and passTT) : 
      ...
      if len(event.FatjetAK08ungroomed_pt) > 0 and len(event.Jet_pt) > 3:
        ...
        #total lhewtcentral for all passing events
          ...
          if i == 5:
            passevtwtpdfUp += float(lhewts[4]) 
          elif i == 6:
            passevtwtpdfLo += float(lhewts[5])
