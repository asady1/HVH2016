...
#        radmasses = [0.75,0.8,1.0,1.2,1.4,1.6]#,3.5,4.,4.5]
        radmasses = [0.75,0.8,1.0,1.2,1.4,1.6,1.7,1.8,1.9,2.0,2.2,2.35,2.5,2.75,3.]
    print radmasses
...
#        for  i in range(0,6):
    ...
    for j in range(0,len(fChain)):
        ...
        #print "%f %f %f %f %f" %(radmasses[j],yobs[j],ymean[j],y1up[j],y1down[j])
        print str(radmasses[j]) +" & " + str(round(yobs[j],2)) + " & " + str(round(ymean[j],2)) + " & " + str(round(y1up[j],2)) + " & " + str(round(y1down[j],2))# + " & " + str(round(y2up[j],2)) + " & " + str(round(y2down[j],2))
    ...
    mg.SetTitle("")
    mg.Draw("AP")
    mg.GetXaxis().SetTitle("m_{X}^{spin-0} (TeV)")
    #resonance="G_{RS}"
    resonance="G_{R}"
    if withAcceptance:
    ...
    mg.GetYaxis().SetRangeUser(1.,5000)
    mg.GetXaxis().SetRangeUser(0.75,1.6) 
 ...
 #   ftheory=open("bulk_graviton_exo15002.txt")	
    ...
    mg.Add(gtheory,"L")
    gtheory.Draw("L")
    ltheory="G_{R} #rightarrow HH (k/#bar{M}_{Pl}=0.1)"
#    ltheory ="G_{RS} (k/M_{Pl} = 0.5)"	
...
#       leg = rt.TLegend(0.59,0.65,0.95,0.89)
       leg = rt.TLegend(0.49,0.55,0.95,0.89)
    ...
    if withAcceptance:
        ...
        c1.SaveAs("brazilianFlag_Rad_2p11p1_%s_13TeV_fix.root" %label)
        c1.SaveAs("brazilianFlag_Rad_2p11p1_%s_13TeV_fix.pdf" %label)
        c1.SaveAs("brazilianFlag_Rad_2p11p1_%s_13TeV_fix.C" %label)
...
if __name__ == '__main__':
  ...
  for chan in channels:
    print "chan =",chan
    masses =[750,800,1000,1200,1400,1600,1700,1800,1900,2000,2200,2350,2500,2750,3000]
