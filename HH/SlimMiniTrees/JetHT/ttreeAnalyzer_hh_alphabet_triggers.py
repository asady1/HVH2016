...
f =  ROOT.TFile.Open(inputFile.replace('root://cmseos.fnal.gov//store/user/mkrohn/HHHHTo4b/V23/JetHT_old/',''), 'recreate')
...
mynewTree.Branch('HLT_PFJet450_v', HLT_PFJet450_v, 'HLT_PFJet450_v/F')
...
#    files = Files_list[i]
	...
	count = count + 1
        ...
        if abs(treeMine.jet1_puppi_eta) > 2.4:
            continue

        if abs(treeMine.jet2_puppi_eta) > 2.4:
            continue


        if abs(treeMine.jet1_puppi_eta-treeMine.jet2_puppi_eta) > 1.3:
            continue
        ...
        if treeMine.jet1_puppi_pt < 200:
            continue

        if treeMine.jet2_puppi_pt < 200:
            continue
