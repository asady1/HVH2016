...
int Display_SignalFits_LL_Data_Radion(std::string dir_preselection="outputs/datacards/",
                       ...
                       bool focus=false)
{
	...
	if (antitag)
		{
			std::cout<<"looking for "<<("vh/Signal_mX_antitag_"+masses.at(i)+"HH_LL_Data_Radion").c_str()<<std::endl;
			h_mX_SR=(TH1D*)file->Get(("vh/Signal_mX_antitag_"+masses.at(i)+"HH_LL_Data_Radion").c_str());
			h_mX_SR->SetTitle(("m_{X} Peak in Signal MC (m_{X}="+masses.at(i)+" GeV); m_{X} (GeV)").c_str());
		}
	else
		{
			std::cout<<"looking for "<<("Signal_mX_"+masses.at(i)+"_HH_LL_Data_Radion").c_str()<<std::endl;
			h_mX_SR=(TH1D*)file->Get(("vh/Signal_mX_"+masses.at(i)+"_HH_LL_Data_Radion").c_str());
			h_mX_SR->SetTitle(("m_{X} Peak in Signal MC (m_{X}="+masses.at(i)+" GeV); m_{X} (GeV)").c_str());
		}
