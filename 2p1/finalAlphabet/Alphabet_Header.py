...
def FillPlots(Alphabet, V, N, NU, ND, A, variable, binBoundaries, AT, T, subtract,TTNom):
	Alphabet.MakeEstVariable(variable, binBoundaries, AT, T)
	for i in Alphabet.hists_MSR:
		#V = veritas = data
		V.Add(i,1.)
	for i in Alphabet.hists_EST:
		#N = antitag*rate calculated = estimate
		N.Add(i,1.)
	for i in Alphabet.hists_EST_UP:
		#NU = estimate up (errors on fit)
		NU.Add(i,1.)
	...
	for i in Alphabet.hists_ATAG:
		...
		A.Add(i,1.)
        if subtract > 0:
		for i in Alphabet.hists_EST_SUB:
			#subtracting ttbar (i) from N
			N.Add(i,-1.)
		for i in Alphabet.hists_MSR_SUB:
			#adding ttbar + estimate + up + down - to calculate total height
			TTNom.Add(i,1.)
			N.Add(i,1.)
			NU.Add(i,1.) 
			ND.Add(i,1.)
		for i in Alphabet.hists_EST_SUB_UP:
			#subtract i from UN
			NU.Add(i,-1.)
		for i in Alphabet.hists_EST_SUB_DN:
			ND.Add(i,-1.)
...
def FillPlotsSys(Alphabet, V, N, NU, ND, A, variable, binBoundaries, AT, T, subtract,TTNom,TTSys):
	Alphabet.MakeEstVariable(variable, binBoundaries, AT, T)
	for i in Alphabet.hists_MSR:
		#V = veritas = data
		V.Add(i,1.)
